...
/*"PARTEII": Altere a Rotina de servico a interrupcao de transmissao de modo a copiar para a UART1 todos os carateres do buffer circular de transmissao ou copiar ate o FIFO da UART1 ficar cheio*/
void _int_(24) isr_uart1(void){
	...
	if(IFS0bits.U1TXIF==1){
		...
		if(txb.count>0){
			/*"copiar para a UART1 todos os carateres do buffer circular de transmissao ou copiar ate o FIFO da UART1 ficar cheio"*/
			/*O bit UTXBF (Transmit Buffer Full) do registo UxSTA e um bit de status que quando ativo, indica que o FIFO de transmissao esta cheio*/
	...
	/*If U1RXIF is set*/
	/*"PARTE II": Copiar para o buffer de rececao todos os carateres disponiveis no FIFO de rececao da UART*/
	if(IFS0bits.U1RXIF==1){
		/*Read character from UART and write it to the "tail" position of the reception buffer*/
	/*O bit URXDA (Receive Data Available) indica, quando ativo, que está disponivel para ser lido pelo menos 1 carater no FIFO de rececao*/
		while(rxb.count<BUF_SIZE && U1STAbits.URXDA==1){
